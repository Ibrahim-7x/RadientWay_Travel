...
export const tourPackages = packages.filter((p) => !isUmrahPackage(p))

export const umrahPackages = packages.filter(isUmrahPackage)

export const featuredPackages = packages.filter((p) => p.featured)

export const getPackageBySlug = (slug) => packages.find((p) => p.slug === slug)

// Region chips on /tours — Umrah has its own page, so it never contributes one.
export const regions = ['All', ...Array.from(new Set(tourPackages.map((p) => p.region)))]
